...
public class TestCase_SpotRateandBooking_002 extends BaseClass {

	@Test(priority = 0)
	public void verifyspotrateandbooking() throws Exception

	{
		System.out.println("***************TestCase_SpotRateandBooking_002*******************");

		// Start Recording
		ScreenRecorderUtil.startRecord("verifyspotrateandbooking()");
		...
		String paymentterms = getCellData(1, 16, "TestData2");
		...
		if (paymentterms.equals("Advance Payment at the time of Booking Confirmation")) {
			...
			hp.setpaymenttermsdays(getCellData(1, 17, "TestData2"));
			Thread.sleep(2000);
		...
		// Open Hash Move Customer Portal URL
		driver.get("https://prehubacme.hashmove.com/login");
		Thread.sleep(10000);
		...
		bpp.setreferencenumber(getCellData(1, 18, "TestData2"));
		logger.info("Reference Number = " + cell.getStringCellValue() + " has entered");

		// Enter Shipping Instructions
		bpp.setshippinginstructions(getCellData(1, 19, "TestData2"));
		...
		// Select SHIPPER NAME
		bdp.setshippername(getCellData(2, 20, "TestData2"));
		...
		bdp.setmainconsigneename(getCellData(2, 26, "TestData2"));
