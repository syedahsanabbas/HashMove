...
//						********* TestCase_UserCreation_005 *********
//		Description: Verify that Add new user with role = Finance on ELM Portal

public class TestCase_UserCreation_005 extends BaseClass {

	@Test(priority = 0)
	@Description("Verify that Add new user with role = Finance on ELM Portal")
	@Severity(SeverityLevel.CRITICAL)
	@Epic("EPUC005")
	@Feature("Feature: Add new user with role = Finance on ELM Portal")
	@Story("Story: Create New User")
	public void verifycreatefinanceuser() throws InterruptedException, IOException

	{
		...
		// Open Hash Move URL
				...
				// Enter User ID
				lp.setuserid(getCellData(5, 1, "UserCreationTestData"));
				logger.info("User ID = " + cell.getStringCellValue() + " has entered");

				// Enter Password
				lp.setpassword(getCellData(5, 2, "UserCreationTestData"));
				...
				// Enter First Name
				alp.setfirstname(getCellData(5, 3, "UserCreationTestData"));
				logger.info("First Name = " + cell.getStringCellValue() + " has entered");
				Thread.sleep(1000);

				// Enter Last Name
				alp.setlastname(getCellData(5, 4, "UserCreationTestData"));
				logger.info("Last Name = " + cell.getStringCellValue() + " has entered");
				Thread.sleep(1000);

				// Enter City
				alp.setcity(getCellData(5, 5, "UserCreationTestData"));
				logger.info("City = " + cell.getStringCellValue() + " has entered");
				Thread.sleep(2000);

				// Enter Contact
				alp.setcontact(getCellData(5, 6, "UserCreationTestData"));
				logger.info("Contact = " + cell.getStringCellValue() + " has entered");
				Thread.sleep(1000);

				// Enter Email
				alp.setemail("test" + alp.getSaltString() + "@" + getCellData(5, 7, "UserCreationTestData"));
				logger.info("Email = " + cell.getStringCellValue() + " has entered");
				Thread.sleep(1000);

				// Select User Role
				alp.setuserrole(getCellData(5, 8, "UserCreationTestData"));
				logger.info("User Role = " + cell.getStringCellValue() + " has entered");
				Thread.sleep(1000);

				// Select Business Unit
				alp.clickbusinessunitdropdown();
				alp.setbusinessunit(getCellData(5, 9, "UserCreationTestData"));
				Thread.sleep(1000);
