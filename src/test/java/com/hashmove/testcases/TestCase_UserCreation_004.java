...
//						********* TestCase_UserCreation_004 *********
//		Description: Verify that Add new user with role = user on ELM Portal

public class TestCase_UserCreation_004 extends BaseClass {

	@Test(priority = 0)
	@Description("Verify that Add new user with role = user on ELM Portal")
	@Severity(SeverityLevel.CRITICAL)
	@Epic("EPUC004")
	@Feature("Feature: Add new user with role = user on ELM Portal")
	@Story("Story: Create New User")
	public void verifycreateuser() throws InterruptedException, IOException

	{
		System.out.println("***************TestCase_UserCreation_004*******************");
		...
		// Open Hash Move URL
				...
				// Enter User ID
				lp.setuserid(getCellData(4, 1, "UserCreationTestData"));
				logger.info("User ID = " + cell.getStringCellValue() + " has entered");

				// Enter Password
				lp.setpassword(getCellData(4, 2, "UserCreationTestData"));
				...
				// Enter First Name
				alp.setfirstname(getCellData(4, 3, "UserCreationTestData"));
				logger.info("First Name = " + cell.getStringCellValue() + " has entered");
				Thread.sleep(1000);

				// Enter Last Name
				alp.setlastname(getCellData(4, 4, "UserCreationTestData"));
				logger.info("Last Name = " + cell.getStringCellValue() + " has entered");
				Thread.sleep(1000);

				// Enter City
				alp.setcity(getCellData(4, 5, "UserCreationTestData"));
				logger.info("City = " + cell.getStringCellValue() + " has entered");
				Thread.sleep(2000);

				// Enter Contact
				alp.setcontact(getCellData(4, 6, "UserCreationTestData"));
				logger.info("Contact = " + cell.getStringCellValue() + " has entered");
				Thread.sleep(1000);

				// Enter Email
				alp.setemail("test" + alp.getSaltString() + "@" + getCellData(4, 7, "UserCreationTestData"));
				logger.info("Email = " + cell.getStringCellValue() + " has entered");
				Thread.sleep(1000);

				// Select User Role
				alp.setuserrole(getCellData(4, 8, "UserCreationTestData"));
				logger.info("User Role = " + cell.getStringCellValue() + " has entered");
				Thread.sleep(1000);

				// Select Business Unit
				alp.clickbusinessunitdropdown();
				alp.setbusinessunit(getCellData(4, 9, "UserCreationTestData"));
				Thread.sleep(1000);
